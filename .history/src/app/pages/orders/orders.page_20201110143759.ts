...
export class OrdersPage implements OnInit {
  ...
  async downloadAsPDF() {
    ...
    doc.setFontSize(18)
    doc.text(this.order.idTrack, 4, 7, options)
    doc.setFontSize(12)
    doc.text(this.order.createdAt.toLocaleString(), 4, 8, options)
    doc.text(this.order.branch.address, 4, 8.5, options)
    doc.setLineWidth(0.1)
    doc.line(0.8, 9, 7.2, 9)
    const prods = []
