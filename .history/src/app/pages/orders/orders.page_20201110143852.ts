...
export class OrdersPage implements OnInit {
  ...
  async downloadAsPDF() {
    ...
    doc.setFontSize(18)
    doc.text(this.order.idTrack, 4, 10, options)
    doc.setFontSize(12)
    doc.text(this.order.createdAt.toLocaleString(), 4, 11, options)
    doc.text(this.order.branch.address, 4, 11.5, options)
    doc.setLineWidth(0.1)
    doc.line(0.8, 12, 7.2, 12)
    const prods = []
