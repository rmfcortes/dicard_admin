...
export class OrdersPage implements OnInit {
  ...
  async downloadAsPDF() {
    ...
    doc.setFontSize(18)
    doc.text(this.order.idTrack, 4, 6.7, options)
    doc.setFontSize(12)
    doc.text(this.order.createdAt.toLocaleString(), 4, 7.3, options)
    doc.text(this.order.branch.address, 4, 7.8, options)
    doc.setLineWidth(0.1)
    doc.line(0.8, 9.3, 7.2, 9.3)
    const prods = []
