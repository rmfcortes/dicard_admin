...
export class OrdersPage implements OnInit {
  ...
  async downloadAsPDF() {
    ...
    const doc = new jsPDF({
      ...
      unit: 'cm',
      format: [(prods.length * 1.5) + 9.5, this.ticketWidth]
    })
    ...
    doc.addImage(this.logo, 'JPEG', 1, 0.2, 6, 6)

    doc.setFontSize(18)
    doc.text(this.order.idTrack, 4, y, options)
    doc.setFontSize(12)
    doc.text(date.toLocaleString(), 4, y + 0.5, options)
    doc.text(this.order.branch.address, 4, y + 1, options)
    doc.setLineWidth(0.1)
    doc.line(0.8, y+2.5, 7.2, y+2.5)
    autoTable(doc, {
      theme: 'grid',
