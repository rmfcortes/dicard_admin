...
export class OrdersPage implements OnInit {
  ...
  async downloadAsPDF() {
    ...
    const doc = new jsPDF({
      ...
      unit: 'cm',
      format: [(prods.length * 1.5) + 9.5, this.ticket.width]
    })
    const options: TextOptionsLight = {
      align: 'center',
      maxWidth: this.ticket.width - 1.6,
    }

    const date = new Date(this.order.createdAt)

    const y = 6.5
    doc.addImage(this.ticket.base64, 'JPEG', 0.8, 0.1, this.ticket.width - 1.6, this.ticket.width - 1.6)
    ...
    autoTable(doc, {
      ...
      margin: 0.5,
      tableWidth: this.ticket.width - 1.6,
      startY: y+3,
