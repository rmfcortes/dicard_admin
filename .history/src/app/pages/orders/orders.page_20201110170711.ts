...
export class OrdersPage implements OnInit {
  ...
  async downloadAsPDF() {
    ...
    doc.line(0.4, y + 0.5, this.ticket.width - 0.4, y + 0.5)
    doc.setFontSize(fontSize + 4)
    doc.text(date.toLocaleString(), this.ticket.width / 2, y + 1, options)
    doc.text(this.order.branch.address, this.ticket.width / 2, y + 1.5, options)
    autoTable(doc, {
      theme: 'grid',
      styles: {fillColor: [0,0 ,0], lineColor: [0, 0, 0], lineWidth: 0.02, valign: 'middle', halign: 'center', fontSize},
      margin: 0.4,
