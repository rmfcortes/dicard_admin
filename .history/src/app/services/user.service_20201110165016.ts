...
import { MainProfile, Ticket } from '../interfaces/profile.interface';
...
export class UserService {
  ...
  getLogo(uid?): Promise<Ticket> {
    return new Promise((resolve, reject) => {
      if (!uid) uid = this.uidService.getUid()
      const logoSub = this.db.object(`principal/${uid}/ticket`).valueChanges()
          .subscribe((logo: Ticket) => {
            logoSub.unsubscribe()
            resolve(logo)
          },
          err => {
